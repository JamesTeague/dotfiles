return {
  {
    ...
    config = function()
      ...
      vim.keymap.set('n', '<leader>gt', [[:let $TEST_ENV = 'local' | GoTestFunc<CR>]],
        { desc = 'Run Nearest [G]o [T]est' })
      vim.keymap.set('n', '<leader>gts', [[:let $TEST_ENV = 'local' | GoTestFunc -s <CR>]],
        { desc = 'Select Go Tests to run' })
      vim.keymap.set('n', '<leader>gtf', [[:let $TEST_ENV = 'local' | GoTestFile <CR>]],
