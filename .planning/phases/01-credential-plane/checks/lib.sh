...
#
# Shared verification helpers for Phase 1 credential-plane checks.
#
...
#   assert_no_grep PATTERN PATH         - NEW for Phase 1: pass when grep -q
#                                         does NOT match; fail if pattern present;
#                                         pending if file missing. Used for SEC-15
#                                         to assert VaultWarden-free templates.
...
# NEW for Phase 1 — inverse of assert_grep.
# Pass when grep finds NO match (pattern absent is correct state — used by
# SEC-15 to assert that VaultWarden calls are absent from apply-time paths).
# Pending when file is missing (file absence is a separate upstream check).
# Fail when pattern IS present (invariant violated — VW reference crept in).
assert_no_grep() {
  local pattern="$1"
  local path="$2"
  if [[ ! -f "${path}" ]]; then
  ...
  if grep -qE -- "${pattern}" "${path}"; then
    fail "grep '${pattern}' FOUND in ${path} (must be absent)"
  else
    pass "grep '${pattern}' not present in ${path}"
  fi
}
